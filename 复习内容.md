js
    ...
    防抖 节流
    预加载（prefetch， preload，script-async-defer）和懒加载（intesectionObserver）
    事件循环 eventloop 
        微任务分类 
        宏任务
...
react(一个周末)
    ...
    说下React原理你理解多少，，React调度机制的优先级是怎么样的？
    Hook你知道多少，useMemo和useCallBack说说呢？
    
    说说Diff算法的过程 讲讲Diff算法（讲下这个场景下diff是怎么计算的）
    React 父组件渲染，子组件一定会渲染么(涉及性能优化)
    说说React.memo()
    实现一个创建节点树
    vue和react的区别
    React的整个更新过程可以聊一聊么（Schedule，setState过程）
    ...
    说说React的事件合成机制
    React事件拦截是在捕获的时候还是冒泡的时候
    React 中是如何处理事件机制的？
    React17 事件委托发生了什么变化？一般用来处理什么场景？

    讲一讲闭包陷阱
    你提到的 useEvent 了解是怎么实现的了解吗
